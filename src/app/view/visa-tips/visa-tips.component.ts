...
export class VisaTipsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.countryShow =JSON.parse(localStorage.getItem('countrylist'));
		...
		if(this.currentIdBlong!='' && this.currentIdBlong!=null && this.currentIdNead!='' && this.currentIdNead!=null)
		{
			this.visafor()
			var cmd=this;
			let BelongToObj = this.belong_to.filter(function(list){ return list.slug_country_name==cmd.currentIdBlong;});
			...
			let NeedVisaForObj = this.need_visa_for.filter(function(list){ return list.slug_country_name==cmd.currentIdNead;});
